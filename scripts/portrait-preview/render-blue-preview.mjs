...
const asset = (name) => `${root}public/assets/portrait/blue/${name}-v1.png`;
const sample = `${root}docs/designs/d062-woman-cutout-v3.png`;
const output = `${root}docs/designs/d062-blue-complete-v2.png`;
const size = 720;
...
const [background, person, confetti, sparkles, gift, star, cake] = await Promise.all([
  fit(asset("background"), 620),
  sticker(sample, 510),
  fit(asset("confetti"), 90),
  fit(asset("sparkles"), 76),
  fit(asset("gift"), 104),
  fit(asset("star"), 150),
  fit(asset("cake"), 245),
]);
const art = await sharp({ create: { width: size, height: size, channels: 4, background: { r: 255, g: 255, b: 255, alpha: 0 } } })
  .composite([
    { input: background, left: 50, top: 0 },
    { input: person, left: 105, top: 122 },
    { input: confetti, left: 114, top: 190 },
    { input: sparkles, left: 530, top: 210 },
    { input: gift, left: 78, top: 315 },
    { input: star, left: 25, top: 505 },
    { input: cake, left: 450, top: 420 },
  ]).png().toBuffer();
